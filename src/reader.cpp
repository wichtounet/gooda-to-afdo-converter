...
std::vector<string_view> parse_gooda_line(std::string& line){
...
bool read_asm_file(const std::string& directory, std::size_t i, converter::Data& data){
...
bool read_src_file(const std::string& directory, std::size_t i, converter::Data& data){
    auto& function = data.functions[i];
    ...
    if(converter::exists(src_file_name)){
        ...
        std::cout << src_file_name << std::endl;
        ...
        while(line.size() > 3){
            ...
            auto line_number = get_counter(contents, SRC_LINE);
...
bool converter::read_spreadsheets(const std::string& directory, converter::Data& data){
    ...
    for(std::size_t i = 0; i < data.functions.size(); ++i){
        if(!read_asm_file(directory, i, data)){
            return false;
        }
        
        if(!read_src_file(directory, i, data)){
            return false;
        }
    }
